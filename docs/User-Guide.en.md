...
Legacy and vzLogger use the same central meter-template catalog. Baud rates are stored neutrally as the initial communication baud rate and read baud rate. Legacy maps these values to `STARTBAUDRATE` and `BAUDRATE`, while vzLogger uses `baudrate` and `baudrate_read`. Meter models, serial settings, and Legacy-specific sequences therefore need to be maintained in only one place.
