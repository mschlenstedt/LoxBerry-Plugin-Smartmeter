...
my $log_file = "$home/log/plugins/$psubfolder/vzlogger.log";
...
my $interval = cron_to_seconds($plugin_cfg->param("MAIN.CRON") || "5");
...
foreach my $config_key (sort keys %flat_config) {
	...
	if ($protocol eq "sml") {
		$meter_config->{aggtime} = $interval;
	} else {
		$meter_config->{interval} = $interval;
...
my $config = {
	verbosity => clean_number($plugin_cfg->param("VZLOGGER.VERBOSITY"), 5),
	log => $log_file,
	retry => 30,
	local => {
