...
- Store channel state in `vzlogger_channel_definitions.json`, migrate legacy selections/custom identifiers atomically while retaining unambiguous legacy cache/UDP names as runtime compatibility aliases, keep plugin display/output metadata out of `vzlogger.conf`, and restrict `vzlogger_channels.json` to active plugin outputs with unique primary cache/UDP keys.
