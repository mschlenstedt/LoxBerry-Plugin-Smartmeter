...
- Remove the separate MQTT-bridge debug switch (`VZLOGGER.DEBUG`). Bridge debug output is now emitted at log level 7 (Debug) through the central log level; the setting is dropped from the configuration on upgrade. The vzLogger service verbosity keeps its own switch because it configures the external `vzlogger` binary. The per-service "Show log" buttons now open the most recent timestamped file; the diagnostic debug-log report is unchanged.

- Remove the Legacy implementation completely. The Perl reader (`fetch.pl`, `sm_logger.pl`, the SML parsers, the reboot cron runner), the Legacy configuration page, its manual-read and log-view CGIs, and all Legacy templates, language phrases, cron handling, and runtime locks are gone. vzLogger is now the only meter implementation and can be active or inactive. Legacy is only maintained in the `Version1` branch.
- On upgrade, obsolete Legacy polling cron entries are removed, the Legacy-only settings `CRON` and `SENDMQTT` are dropped from `smartmeter.cfg`, and a stored `IMPLEMENTATION=legacy` becomes `IMPLEMENTATION=none` so meter reading has to be re-enabled explicitly with vzLogger. The `libdevice-serialport-perl` dependency is no longer installed.
- Restore the original LoxBerry plugin identity (`AUTHOR.NAME`, `AUTHOR.EMAIL`) and point the automatic-update URLs back at the upstream repository. The plugin is renamed to Smartmeter-NG (`PLUGIN.NAME`/`PLUGIN.FOLDER` = `smartmeter-ng`) at version 2.0; the MQTT bridge service is now `smartmeter-ng-vzlogger-bridge.service` and the vzLogger drop-in is `smartmeter-ng.conf`.
